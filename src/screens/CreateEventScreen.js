...
import { showToastMSGInfo, showToastMSGWarning } from '../utils/ToastMessages';
const CreateEventScreen = () => {
...
const removeHost = (id) => {
  ...
  return (
      ...
      <ScrollView
        ...
        <View
          ...
          <View
            ...
            <Text style={{color: '#9CA2FF', fontSize: 13}}>341 x 360</Text>
          </View>
          <TextInput
            style={{
              fontSize: 18,
    ...
    <View key={host.id} style={{gap: 10,}}>
          ...
          <Text style={{color: '#4A4A4A', fontSize: 16}}>Host {index + 1}</Text>
          {hosts.length > 1 && index !== 0 && (
  <TouchableOpacity onPress={() => removeHost(host.id)} style={{alignSelf: 'flex-end', marginBottom: 8}}>
    <DeleteIcon />
  </TouchableOpacity>
)}
          </View>
        ...
        <TouchableOpacity style={{backgroundColor:"#6D5CFF", paddingVertical:14, paddingHorizontal:10,borderRadius:12, elevation:1}}>
          <Text style={{fontSize:18, color:"#FFFFFF", fontWeight:600, textAlign:"center"}}>Save & Publish</Text>
        </TouchableOpacity>
         <View
                  style={{
                    flexDirection: 'row',
