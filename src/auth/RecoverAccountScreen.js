...
import {FORGOT_PASSWORD_API, SIGNUP_API, SIGNWITHGOOGLE_API} from '../utils/ApiHelper';
...
const RecoverAccountScreen = ({navigation}) => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const response = await axios.post(FORGOT_PASSWORD_API, data, config);
      setLoading(false);
      ...
      if (response.data.success) {
          ...
          setTimeout(() => {
            navigation.navigate('OTPScreen', {email, flow:"ForgotPassword"});
          }, 2000);
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <View style={styles.content}>
          <Image
            source={require('../assets/DRIP_11.png')}
            style={styles.illustration}
            resizeMode="contain"
          />

          <Text style={styles.title}>Let’s Recover Your Account </Text>
